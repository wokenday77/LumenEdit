...
struct CameraView: View {
    ...
    /// 手势几何参数（2026-09-18 按真机反馈调过三轮，别再照着原型原样抄）。
    ///
    /// **为什么第一轮放宽**：原型是 24 起手 / 34 位移 / 0.8s / 起点 55% —— 那是浏览器里
    /// 用鼠标/触屏双环境定的口径，搬到真机上偏严。真机反馈"上滑要很用力 + 只有特定位置能触发"，
    /// 算完账发现根因不是"力度"，而是**可用起手带只有约 6pt**（当时参数排常驻把底栏撑到 334pt，
    /// 起点线 428 与底栏上沿 434 只差 6pt）。
    ///
    /// **为什么第三轮再收**（2026-09-18 下午 · Mac 侧复验实测）：放宽 + 提前触发 + 挂整页之后
    /// 先是**过头**（"太敏感"），回调到 16 → 20 后复验**仍在敏感侧** ——
    /// Mac 的实值是 20pt ≈ 4.4mm，而心理预期是 5~6mm 量级，于是定稿 **28pt ≈ 6.2mm**。
    /// 起手门槛 10 → 14 保留（它负责推迟"进入判定"的时机，阈值负责"多远才算数"）。
    ///
    /// ⚠️ **`swipeMaxDuration` 刻意保持 1.2s 不回调**：它不是敏感度参数，而是**宽容度**参数 ——
    /// 它是"超时不算滑动"的**上限**，调小只会排斥慢滑，正是第一轮修掉的
    /// "要很用力/滑快才认"的老问题。降敏感不能从它下手。
    private static let swipeActivationDistance: CGFloat = 14
    /// 位移阈值（**28pt ≈ 6.2mm**，2026-09-18 第三次真机复验后定稿）。
    ///
    /// 演进：34（原型，偏硬）→ 18 → 16（第一轮"太严"）→ 20（第二轮"太灵"）→ **28**。
    /// Mac 侧复验口径："心理预期是 5~6mm 量级" —— 28pt 正落在
    /// **34（僵硬）与 16（过敏）的中间偏硬**处。
    ///
    /// ⚠️ 28 能成立的前提有两个，缺一不可：
    ///   1. **方向锁**已拦掉横滑类误触（否则阈值必须压到 16 以下才够灵敏）
    ///   2. **提前触发**（滑够即生效、不等松手）—— 否则 28pt 又要"滑到底再松手"才认，回到僵硬
    private static let swipeMinTravel: CGFloat = 28
    ...
    /// 方向锁的起手死区（超过它才定主轴）。起手门槛是 14pt，所以第一次回调就能定下来；
    /// 6 → **10**（第三轮）：死区大一点，主轴判定更明确，减少"起手那一下还没定性"的模糊期。
    private static let swipeAxisDeadZone: CGFloat = 10
