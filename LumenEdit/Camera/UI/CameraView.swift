...
struct CameraView: View {
    ...
    ///
    /// **为什么第二轮回调**（2026-09-18 下午）：放宽 + 提前触发 + 挂整页之后**过头了** ——
    /// 真机反馈"上下划拉出/拉回面板有点敏感"。回调的是**位移阈值**（16 → 20）与
    /// **起手门槛**（10 → 14），两者配合：门槛推迟"进入判定"的时机，阈值决定"多远才算数"。
    ///
    ...
    /// 位移阈值（第三轮定稿 **20**，约 2mm）。
    /// 演进：34（原型）→ 18 → 16（第一轮"太严"）→ **20**（第三轮"太灵"）。
    /// 20 能成立的前提是**方向锁**已经拦掉了横滑类误触 —— 没有方向锁时，
    /// 阈值必须压到 16 以下才够灵敏；有了它，阈值可以回到"明确意图"的量级。
    private static let swipeMinTravel: CGFloat = 20
