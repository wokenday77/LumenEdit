...
/// | 对焦 | **展开 / 收起对焦圆盘**（B3b，能力分派） | 虚拟多摄不支持手动对焦 → toast 不开盘（诚实边界）；物理镜头架构落地后自动开。点取景器任意位置**仍可自动对焦**；手动锁定期间点按 = 只测光（拍板 ③） |
...
/// | 曝光补偿 | **展开 / 收起 EV 圆盘** | 模块 #8（B3a：模态，±3 EV / 点数值框归零） |
/// | 设置 | **真开设置页** | 与顶栏齿轮进的是同一个 `SettingsSheet` |
///
/// ⚠️ **三条刻度条的手动开关受能力探测约束**（2026-09-19 起）：虚拟多摄不支持手动
/// 参数（SDK `AVCaptureDevice.h:538-541`），开关在 `ParameterStripView` 里置灰、
/// 点了给原因 —— 本格只负责展开刻度条（看当前值永远可用），不做二次置灰。
///
/// ## 高亮态（B2b 新增）
///
