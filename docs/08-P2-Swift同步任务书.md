...
| **P2 UI 骨架** | 🔶 **进行中** —— 2-1 / 2-1b / 2-2（含模式条改版）/ 模式条可用性修正 / 2-3（焦段条）/ **2-4（底部图标行）** 已交付；2-5 待做 |
...
| **2-5** | `ShutterRowView`（快门排四件套 + ⤢） | **2-5a 已交付**（`Camera/UI/ShutterRowView.swift`）：四件套常态版式 + 尺寸修正（快门 74→60 / 环 5→3.5 / 内芯固定 46 / 缩略图 52→50）+ **决策 #2 视频缩略图修复**（`ThumbnailCache` 对 MOV 改用 `AVAssetImageGenerator` 抽首帧）。**2-5b（⤢ 放大态）待做**。⚠️ 一处方案外的偏差：**录制计时徽标挪到取景器顶部居中**（原位置被 2-3 的焦段条占了，见 `docs/09` 附录） | 真机截图：**四件套版式**；快门**居中**；录制态（红方块）正常；**录完视频左下角缩略图有图** |
